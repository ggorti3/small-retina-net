...
def generate_coco_eval_files(base_path="Object-Detection-Metrics-master/groundtruths",
                             ann_path='/srv/datasets/coco/annotations/instances_val2017.json'):
    testing_data = dset.CocoDetection(root = '/srv/datasets/coco/val2017',
                        annFile = ann_path,
                        transform=transforms.ToTensor())
    for i, (x, y) in enumerate(testing_data):
        #path = "groundtruths/img{}.txt".format(i)
        path = base_path + "/img{:010d}.txt".format(i)
        f = open(path, 'w')
        
        x_scale = 300 / x.shape[2]
        y_scale = 300 / x.shape[1]
        ...
        for elem in y:
            ...
            name = INV_COCO_DICT[elem["category_id"]].replace(" ", "")
            
            info_string = "{} {} {} {} {}\n".format(name, x1, y1, w, h)
            #print(info_string)
            f.write(info_string)
        f.close()
#         if i == 5:
...
def generate_voc_eval_files(save_path, data_path, year, image_set):
    testing_data = dset.VOCDetection(root = data_path,
                                     year = year,
                                     image_set = image_set,
                                     target_transform=voc_target_transform)
    for i, (x, y) in enumerate(testing_data):
        file_path = save_path + "/img{:010d}.txt".format(i)
        f = open(file_path, 'w')
        ...
        for j in range(y.shape[0]):
            name = INV_VOC_DICT[int(y[j, 0])]
            x1 = (y[j, 1] - 0.5 * y[j, 3]) * x_scale
            y1 = (y[j, 2] - 0.5 * y[j, 4]) * y_scale
            w = y[j, 3] * x_scale
            h = y[j, 4] * y_scale
...
#     coco_data = dset.CocoDetection(root = '/srv/datasets/coco/val2017',
#                                    annFile = '/srv/datasets/coco/annotations/instances_val2017.json',
...
#     train_generator = DataLoader(coco_data, batch_size=2, collate_fn=detection_collate_fn)
#     img_batch, label_batch = next(iter(train_generator))
#     print(img_batch)
#     print(label_batch)

#     coco_val_generator = get_coco_generator(img_path='/srv/datasets/coco/val2017',
#                                             ann_path='/srv/datasets/coco/annotations/instances_val2017.json',
#                                             batch_size=3)
#     img_batch, label_batch = next(iter(coco_val_generator))
#     print(img_batch)
#     print(label_batch)
    
#     voc_val_generator = get_voc_generator('/srv/datasets/pascalvoc-2012', "2012", "val", 3)
#     img_batch, label_batch = next(iter(voc_val_generator))
#     print(img_batch)
#     print(label_batch)
    
#     generate_voc_eval_files("Object-Detection-Metrics-master/groundtruths", '/srv/datasets/pascalvoc-2012', "2012", "val")
    generate_coco_eval_files()